...
Feature: Search Client used by MYACC self trigger invite
    ...
    Scenario: both mobile number and email added
		...
		* karate.write(responseStr, '../src/test/java/app/api/reload/services-css/SearchClient/SearchClient-s3-response 24.5.txt')
		Then status 200				
