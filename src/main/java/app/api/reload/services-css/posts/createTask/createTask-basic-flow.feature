...
Feature:
    ...
    Scenario Outline: Create activity <taskType> on <caseNumber>
        ...
        * configure headers  = WS_HTTP_HEADERS_REST
        And request read('classpath:app/api/reload/services-css/posts/createTask/createTask-basic-flow.xml')
        When method post
        * print response
        Then status 200
        ...
        Examples:
        | caseNumber   | taskType								 |
        | 10060080879  | AUTO: You Have Mail Medical notes added |
