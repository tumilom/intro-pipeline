@ignore
@cc_eforms-add-ICP-referral-Decline
Feature: ccapi add an eform

    Background: 		
		# For a unit testing uncomment line below, update value, remove @ignore		
		#* def claimNumber = '10060081872'
		
    Scenario:  ccapi add eforms
		* def path = '/cases/<path-claimNumber>/eforms'	
		* def body = read('classpath:app/api/reload/api-cc/posts/eforms-add-ICP-referal/eforms-add-ICP-referral-Decline.json')
