...
Feature: ccapi add add-client-goals
    ...
    Scenario:  ccapi add add-activities
		* def path = '/claims/<path-claimNumber>/recovery-plans/<path-recoveryPlanId>/client-goals/<path-clientGoalId>/edit'
		* def body = read('classpath:app/api/reload/api-cc/posts/edit-client-goals.json')		
		
		* replace path.path-claimNumber = claimNumber
		* replace path.path-recoveryPlanId = recoveryPlanId
		* replace path.path-clientGoalId = clientGoalId
