@ignore
@cc_eforms-add-ICP-referral-Clinical
Feature: ccapi add an eform

    Background: 		
		# For a unit testing uncomment line below, update value, remove @ignore		
		#* def claimNumber = '10060066385-ICP'
		
    Scenario:  ccapi add eforms
		* def path = '/cases/<path-claimNumber>/eforms'	
		* def body = read('classpath:app/api/reload/api-cc/posts/eforms-add-ICP-referal/eforms-add-ICP-referral-Clinical.json')
