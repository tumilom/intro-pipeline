@ignore
@customer_add-actual-earningsPIEHPW
Feature: customer add an earnings at benefit level
    ...
    Scenario:  add abatement = Post Incapacity Earnings Hours Per Week
		* def path = '/cases/<path-benefitCaseNumber>/occupations/<path-occupationId>/actual-earnings'	
		* def body = read('classpath:app/api/reload/api-customer/ACCCU1036Retest/add-actual-earningsPIEHPW.json')				
