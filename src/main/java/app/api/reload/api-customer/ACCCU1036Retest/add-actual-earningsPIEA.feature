@ignore
@customer_add-actual-earningsPIEA
Feature: customer add an earnings at benefit level
    ...
    Scenario:  add abatement = Post Incapacity Earnings Amount
		* def path = '/cases/<path-benefitCaseNumber>/occupations/<path-occupationId>/actual-earnings'	
		* def body = read('classpath:app/api/reload/api-customer/ACCCU1036Retest/add-actual-earningsPIEA.json')				
