...
Feature:
    ...
    Scenario: ERA Fasttrack 
		...
		* def seRes = call read('classpath:app/api/reload/web/webui-claim-steps.feature@setupERAEarnings'){ employerNumber:'#(employerNumber)', earnings01Amount:'#(earnings01Amount)', earnings02Amount:'#(earnings02Amount)',employmentType:'#(employmentType)',earnings01Type:'#(earnings01Type)',earnings02Type:'#(earnings02Type)',eraPaymentOption:'#(eraPaymentOption)',effectiveDateVal:'#(effectiveDateVal)'}
		...
		* def sfc2Res = call read('classpath:app/api/reload/web/webui-claim-steps.feature@addERAAdjustment'){ caseNo : '#(wceCaseNo)',adjType : '#(adjType)'}		
