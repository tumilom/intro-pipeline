@pic-singleIAfasttrackfromscratch
Feature:
    Background: 
    ...
    Scenario Outline: Fasttrack for ${RUNID}<seq>
		...
		* def injuryType = 'Sensitive'					
		* def eetRes = call read('classpath:app/api/reload/web/webui-claim-pic-steps.feature@addPICInjuryTypeSensitive'){injuryTypeVal : '#(injuryType)',occurredDateVal : '<picAccidentDateNoTime>'}		
	...
	@pic-singleIA-fasttrackfromscratchFullLoad	
	Examples:
	    | read('pic-singleIA-fast-track-data.csv') | 
	
	@pic-singleIA-fasttrackfromscratchSingleCase
	Examples:
	    | read('pic-singleIA-fast-track-data-singlecase.csv') | 