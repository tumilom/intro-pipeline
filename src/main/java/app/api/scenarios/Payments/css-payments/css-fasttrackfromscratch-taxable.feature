@css-fasttrackfromscratch-taxable
Feature:
    Background: 
    ...
    Scenario Outline: Fasttrack for ${RUNID}<seq>
		...
		* def asRes = call read('classpath:app/api/reload/web/webui-claim-steps.feature@addSubcase'){ subcaseType :'Client Support Services'}
		...
		* def cssCaseNo = benefitResult.cssCaseNo		
	
		* print JavaApp.waitBeforeDriverInit()
		* driver url = FRONTOFFICE_URL +'/home.jsp'	
		* def sfc1Res = call read('classpath:app/api/reload/web/webui-claim-steps.feature@searchForCase'){ caseNo : '#(cssCaseNo)'}	
        * def atcRes = call read('classpath:app/api/reload/web/webui-claim-steps.feature@addTaxCodeAtBenefitLevel'){ caseNo : '#(cssCaseNo)',roleName : '<roleName>',taxCode : '<taxCode>',taxRate : '<taxRate>',withHoldingStatus : '<withHoldingStatus>'}						
		* def abaaRes = call read('classpath:app/api/reload/web/webui-claim-steps.feature@addCSSBebefitAmountAdjustment'){ caseNo : '#(cssCaseNo)', adjType : '<adjType>',entitlementName : '<entitlementName>',startDateVal : '#(effectiveDateVal)',unitVal : '<unitVal>',rateVal : '<rateVal>'}
		...
		* def sfc2Res = call read('classpath:app/api/reload/web/webui-claim-steps.feature@searchForCase'){ caseNo : '#(cssCaseNo)'}
		* def awpRes = call read('classpath:app/api/reload/web/webui-claim-steps.feature@approvePayment'){ caseNo : '#(cssCaseNo)'}
		* def ftRes = call read('classpath:app/api/reload/web/webui-claim-steps.feature@fastTrackPayment'){ caseNo : '#(cssCaseNo)'}
	...
	Examples:
	    | read('css-fast-track-data-taxable.csv') |

	
	@css-fasttrackfromscratchSingleCase
	Examples:
	    | read('css-fast-track-data-singlecase-taxable.csv') | 