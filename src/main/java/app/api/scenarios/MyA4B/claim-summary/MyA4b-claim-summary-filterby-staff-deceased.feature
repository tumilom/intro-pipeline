@ignore
@mya4b-claim-summary-filterby-staff-deceased
Feature: Setup user to employer
    ...
    Scenario:      
		...
		* def createGCUserRes = call read('classpath:app/api/reload/services-css/posts/createOrUpdateEmployerViewpointUser.feature') { employerId: '#(employerNo)', userIdToCreate: '#(eosGCUserId)' } 		
		...
		# Filter claim summaries by staff and deceased
		* def eosFilterStaff = 'true'
		* def eosFilterDeceased = 'true'
		* def rcdFilterRsp = call read('classpath:app/api/scenarios/MyA4B/claim-summary/claim-summaries-filterby-staff-deceased.feature') { staffFilter: '#(eosFilterStaff)', deceasedFilter: '#(eosFilterDeceased)', gcUserId: '#(eosGCUserId)'} 