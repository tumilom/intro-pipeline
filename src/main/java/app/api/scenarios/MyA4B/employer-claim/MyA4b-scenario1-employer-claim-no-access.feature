@ignore
@mya4b-scenario1-employer-claim-no-access
Feature: Setup user to employer
    ...
    Scenario:      
		...
		* def rcdRsp = call read('classpath:app/api/scenarios/MyA4B/employer-claim/readDisabilityDetails-no-access.feature') { claimNumber: '#(acc45CaseNo)', gcUserId: '#(eosGCUserId)'} 
		...
		* def rdocRsp = call read('classpath:app/api/scenarios/MyA4B/employer-claim/documents-no-access.feature') { claimNumber: '#(acc45CaseNo)', gcUserId: '#(eosGCUserId)'} 
		
		* def rdcRsp = call read('classpath:app/api/scenarios/MyA4B/employer-claim/diagnosisCodes-no-access.feature') { claimNumber: '#(acc45CaseNo)', gcUserId: '#(eosGCUserId)'} 
		
		* def rbenRsp = call read('classpath:app/api/scenarios/MyA4B/employer-claim/benefits-no-access.feature') { claimNumber: '#(acc45CaseNo)', gcUserId: '#(eosGCUserId)'} 
