@ignore
@mya4b-readClaimsEform-no-access
Feature: Setup user to employer
    ...
    Scenario:      
		...
		* def reformsRsp = call read('classpath:app/api/scenarios/MyA4B/employer-claim/readEform-no-access.feature') { claimNumber: '#(acc45CaseNo)', gcUserId: '#(eosGCUserId)', eformId: '#(eformId)'} 	
		
		* def reformsRsp = call read('classpath:app/api/scenarios/MyA4B/employer-claim/eforms-no-access.feature') { claimNumber: '#(acc45CaseNo)', gcUserId: '#(eosGCUserId)'}
